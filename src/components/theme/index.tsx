...
const CustomThemeProvider = (props: Props) => {
  ...
  // Start with systemMode so SSR and initial client render match, then update after mount
  const [currentMode, setCurrentMode] = useState<SystemMode>(systemMode)

  useEffect(() => {
    const next = settings.mode === 'system' ? (isDark ? 'dark' : 'light') : (settings.mode as SystemMode)

    setCurrentMode(next)
  }, [isDark, settings.mode])
